...
import os, sys
import time
import csv, psycopg2, argparse

def convert(csvpath, dbname=None, tablename=None, sqlpath=None, guessdatatypes=True, samplesize=1000, verbosity=0):
  ...
  with open(csvpath, 'rb') as f:
    ...
    # connect to database
    # TODO: change the dbname to the user given dbname
    with psycopg2.connect("dbname=%s user=postgres" % dbname) as conn:
...
def dump_database(csv_file,dbname):

  args = {
    "dbname": "test",
    "dump_file": os.path.basename(os.path.splitext(csvpath)[0]) + ".dump"
  }

  # Run the shell command to create a dumpfile
  os.system("pg_dump", "--data-only -C -h localhost -U postgres %(dbname) -f %(dump_file)" % args)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Converts a CSV file to a Postgres database')
  parser.add_argument("csv_file", help="path to CSV file")
  parser.add_argument('-d', '--dbname', help='path to Postgres database name (test by default)')
  ...
  convert(args.csv_file, args.dbname, args.table_name, args.sql_create, not args.naive_datatypes, args.sample_size, args.verbosity)

  # TODO: dump the test database to a file
  # dump_database(args.csv_file)
