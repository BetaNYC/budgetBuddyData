...
class OperatingBudgetParser(object):
    ...
    def __init__(self, write_header=False):
        self.classification = {}
        self.columns = u''
...
def scrape(f, write_header=False):

    parser = None
    position_schedule_parser = PositionScheduleParser()
    operating_budget_parser = OperatingBudgetParser(write_header=write_header)
...
if __name__ == '__main__':
    for i, filename in enumerate(sys.argv[1:]):
        write_header = i == 0  # Write header only for first file.
        scrape(open(filename, 'r'), write_header=write_header)
